...
from scipy.misc import imsave, imread, imresize #in newer version of scipy deprecated! use opencv or something for that
...
def letter_seg(lines_img, x_lines, i):
	...
	for e in range(len(letter)):
		if(letter[e][0]<x_linescopy[0]):
			...
			cv2.imwrite('./characters/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png', letter_img) #255-letter_img for inverting
		else:
			x_linescopy.pop(0)
			word += 1
			letter_index = 1
			letter_img_tmp = lines_img[i][letter[e][1]-5:letter[e][1]+letter[e][3]+5,letter[e][0]-5:letter[e][0]+letter[e][2]+5]
			letter_img = cv2.resize(letter_img_tmp, dsize =(28, 28), interpolation = cv2.INTER_AREA)
			cv2.imwrite('./characters/'+str(i+1)+'_'+str(word)+'_'+str(letter_index)+'.png', letter_img)
